...
there is indeed normally a large pile of sand
somewhere near here,
but you don't see anything
besides lots of water and the small island
...
You manage to sign up as a competitor
in the boat races just in time.
The organizer explains that it's not really a traditional race
- instead, you will get a fixed amount of time
during which your boat has to travel as far as it can,
and you win if your boat goes the farthest.

As part of signing up,
you get a sheet of paper (your puzzle input)
...
The organizer brings you over to the area
where the boat races are held.
The boats are much smaller than you expected
- they're actually toy boats,
...
For each whole millisecond
you spend at the beginning of the race holding down the button,
the boat's speed increases by one millimeter per millisecond.

So, because the first race lasts 7 milliseconds, you only have a few options:

    Don't hold the button at all
    (that is, hold it for 0 milliseconds)
...
you could hold the button for 2, 3, 4, or 5 milliseconds
at the start of the race.

In the second race, you could hold the button
for at least 4 milliseconds
and at most 11 milliseconds
and beat the record, a total of 8 different ways to win.

In the third race, you could hold the button
for at least 11 milliseconds
and no more than 19 milliseconds
and still beat the record,
...
you realize the piece of paper
with race times and record distances
you got earlier actually
just has very bad kerning (Unterschneidung).
...
how many ways there are to win this single race.
In this example, the race lasts
for 71530 milliseconds and the record distance
you need to beat is 940200 millimeters. You could hold the button anywhere from 14 to 71516 milliseconds and beat the record, a total of 71503 ways!
...
bool win(
  ...
  unsigned long long race_duration,
  unsigned long long rec_distance);

int main(int argc, char* argv[]){

  //std::ifstream input("res/06_example.txt");
  std::ifstream input("res/06_input.txt");
  std::vector<std::vector<unsigned int>> races{ {}, {} };
  std::vector<unsigned long long> race_p2;
  ...
  while(input.good()){
	...
	if(c>='0' && c<='9'){
	  ...
	  races[line_nr].push_back(std::stoul(num));
	  num.clear();
	}
      } // end for c in line
    if(num.size()) races[line_nr].push_back(std::stoul(num));
    if(num2.size()) race_p2.push_back(std::stoull(num2));
    num2.clear();
    ++line_nr;
  } // end reading
  input.close();
  ...
  for(unsigned int race=0; race < races[0].size(); ++race ){
    unsigned int wins{};
    for(
      unsigned long long hold_duration = 1;
      hold_duration<races[0][race];
      ++hold_duration)
      wins+= win(hold_duration,races[0][race],races[1][race]);
  ...
  for(
      unsigned long long hold_duration = 1;
      hold_duration<race_p2[0];
      ++hold_duration){
    part2+= win(hold_duration,race_p2[0],race_p2[1]);
  }
  
  std::cout << "part1: " << part1 << std::endl; //1083852
  std::cout << "part2: " << part2 << std::endl; //23501589
...
bool win(
  ...
  unsigned long long distance
	= (race_duration - hold_duration) * hold_duration;
  return (distance>rec_distance);
}
