...
The gondola takes you up. Strangely, though,
the ground doesn't seem to be coming with you;
...
The gondola carries you to the surface of the new island
and lurches into the station.

As you exit the gondola,
the first thing you notice is
that the air here is much warmer than it was on Snow Island.
It's also quite humid.
Is this where the water source is?

The next thing you notice
is an Elf sitting on the floor
across the station in what
...
- this is Island Island, after all!
I bet the gardener would know.
He's on a different island, though -
er, the small kind surrounded by water,
not the floating kind.
...
I got all these scratchcards as a gift,
...
Picking one up, it looks like each card
...
one of you realizes that the rules have actually
been printed on the back of every card this whole time.

There's no such thing as "points".
Instead, scratchcards only cause you
to win more scratchcards
...
Copies of scratchcards are scored
like normal scratchcards and have the same card number as the card they copied.
...
This process repeats until none of the copies
cause you to win any more cards.
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11
    ...
    (one original and thirteen copies) have no matching numbers and win no more cards.
...
until no more scratchcards are won.
...
int main(int argc, char* argv[]){
  //Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
  std::regex numbers("((\\d+\\s+)+)\\|((\\s+\\d+)+)");
  std::regex num("(\\d+)");
  std::smatch sm;

  std::ifstream input("res/04_input.txt");
  unsigned int part1{};
  unsigned int part2{};
  unsigned int count{};
  std::vector<unsigned int> cards_count;
  ...
  while(input.good()){
    ...
    if(line.size()){
      ...
      std::regex_search(line,sm,numbers);
      std::string s_win_cards = sm[1].str();
      std::string s_my_numbers = sm[3].str();
      
      std::set<unsigned int> win_cards;
      while (std::regex_search(s_win_cards, sm, num)) {
	win_cards.insert( std::stoul( sm[1].str() ) );
	s_win_cards = sm.suffix().str();
      }
      std::set<unsigned int> my_numbers;
      while (std::regex_search(s_my_numbers, sm, num)){
	my_numbers.insert( std::stoul( sm[1].str() ) );
	s_my_numbers = sm.suffix().str();
      }

      unsigned int points{};
      unsigned int number_of_matches{};
      for(auto n : my_numbers){
	if( win_cards.count(n) ){
	  if(points) points *=2;
	  else points = 1;
      ...
      if(count < cards_count.size()) ++cards_count.at(count);
      else cards_count.push_back(1);
      part2 += cards_count.at(count);
      ++count;
      for(unsigned int i=cards_count.at(count-1);i;--i) //instances of card count
	for(unsigned int n =0; n<number_of_matches;++n)
          if (count + n < cards_count.size()) ++cards_count.at(count + n);
          else cards_count.push_back(1);

    }// end not empty line
  }// end read

  std::cout << "part1: " << part1 << std::endl; //23028
  std::cout << "part2: " << part2 << std::endl; //9236992
