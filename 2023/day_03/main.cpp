...
an engine part seems to be missing from the engine,
...
There are lots of numbers and symbols
...
because they are not adjacent (angrenzend) to a symbol:
114 (top right) and 58 (middle right).
Every other number is adjacent to a symbol
...
The engineer finds the missing part
and installs it in the engine!
As the engine springs to life,
you jump in the closest gondola,
...
int main(int argc, char* argv[]){
  ...
  std::ifstream input("res/03_input.txt");
  //std::ifstream input("res/03_example.txt"); //4361
  while(input.good()){
    std::string line;
    std::getline(input, line);
    if( line.size() ) puzzle.push_back(line);
  }
  input.close();
  ...
  for(unsigned int i=0; i<puzzle.size();++i)
    for(unsigned int j=0;j<puzzle[i].size();++j){
      char num_part = puzzle[i][j];
      if(num_part>='0' && num_part<='9'){
	candidate += num_part;
	for(unsigned int x = i>1? i-1 : 0;x<i+2 && x<puzzle.size();++x)
	  for(unsigned int y = j>1? j-1 : 0;y<j+2 && y<puzzle[i].size();++y){
              if (
		(puzzle[x][y] < '0' || puzzle[x][y] > '9')
		&&
		puzzle[x][y] != '.'
		) is_adjacent = true;
	  } // y
	...
	if( candidate.size() && is_adjacent){
	  sum += std::stoi(candidate);
	} //add adjacent number to sum
	...
	if(puzzle[i][j] == '*'){
	  struct Gearnum{
	    ...
	    unsigned long value{};
	    bool operator<(const Gearnum& o) const {return start<o.start;}
	  };
	  std::set<Gearnum> gear;
	  for(unsigned int x = i>1? i-1 : 0;x<i+2 && x<puzzle.size();++x)
            for (unsigned int y = j > 1 ? j - 1 : 0;
                 y < j + 2 && y < puzzle[i].size(); ++y) {
	      if(puzzle[x][y]>='0' && puzzle[x][y]<='9'){
		// search the start of the number
		...
		if(puzzle[x][column]<'0' || puzzle[x][column]>'9') ++column;
		unsigned long long start = x*puzzle[x].size() + column;
		std::string s_val{puzzle[x][column]};
		char c;
		while(
		  ++column < puzzle[x].size()
		  &&
		  ((c = puzzle[x][column]) >= '0')
		  &&
		  c <= '9'
		  ) s_val += c;
		unsigned long num = std::stoul(s_val);
		Gearnum g {start,num};
		gear.insert(g);
	      } // is num
            } // x,y around *
  ...
  std::cout << "part1: " << sum  << std::endl; //551094
  std::cout << "part2: " << sum2 << std::endl; //80179647
